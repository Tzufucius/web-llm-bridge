...
let stage = 0;
const current = () => stage === 0 ? [make("user", "u2", "当前问题"), make("assistant", "a2", "当前回答")] : stage === 1 ? [make("user", "u1", "旧问题"), make("assistant", "a1", "旧回答"), make("user", "u2", "当前问题"), make("assistant", "a2", "当前回答")] : [make("user", "next", "新会话")];
...
const { context, listeners } = loadContentRuntime({ document, timingOverrides: { historyLoadTimeoutMs: 80, historyPollIntervalMs: 5 }, onScroll: (top) => { if (top < 900) stage = 1; } });
